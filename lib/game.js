...
class Game {
  ...
  collisionDetection(towerX) {
    debugger
    if (this.cab.width + this.cab.cabX - 2 > towerX
      && this.cab.width + this.cab.cabX < towerX + this.obstacles.towers.towerWidth
        && this.cab.cabY - this.cab.height < this.canvas.height - this.obstacles.towers.towerOffsetTop) {
          this.cab.cabX -= 20;
          this.cab.damage += 1;
    }
  }
  ...
  collisionDetector() {
    this.obstacles.allObstacles.forEach( obstacle => {
      this.collisionDetection(obstacle.x);
    });
  }
  ...
  draw() {
    this.obstacles.towers.towerGenerator();
