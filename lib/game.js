...
import { checkAndSetNewHighScore } from './util';
...
class Game {
  constructor(canvas, context) {
    ...
    this.profiler = new PerformanceProfiler('JavaScript Game');
  ...
  processGameOver() {
    ...
    checkAndSetNewHighScore(this.score);
    this.score = 0;
    this.inProgress = false;
    this.over = true;
  }
  ...
  draw() {
    if (this.profilingEnabled) this.profiler.startFrame();
    ...
    if (this.over) {
      ...
      // Game update phase
      if (this.profilingEnabled) this.profiler.startUpdate();
      ...
      if (this.profilingEnabled) this.profiler.endUpdate();
      
      // Render phase
      if (this.profilingEnabled) this.profiler.startRender();
      ...
      if (this.profilingEnabled) this.profiler.endRender();
      
      // Collision detection phase
      if (this.profilingEnabled) this.profiler.startCollisionDetection();
      this.collisionDetector();
      if (this.profilingEnabled) this.profiler.endCollisionDetection();
      
      // Record entity count
      if (this.profilingEnabled) {
        this.profiler.recordEntityCount(this.obstructions.allObstructions.length);
        // Record memory usage every 60 frames
        if (this.profiler.totalFrames % 60 === 0) {
    ...
    if (this.profilingEnabled) this.profiler.endFrame();
  ...
  getProfileData() {
    if (this.profilingEnabled) {
      return this.profiler.exportData();
    }
    return null;
  }
